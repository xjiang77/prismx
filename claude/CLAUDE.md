...
进入项目时，根据 package.json / go.mod / pyproject.toml 检测语言并应用：

### TypeScript (pnpm/bun, vitest/jest)
- 有 bun.lockb 用 bun，否则用 pnpm
- 测试优先 vitest，fallback jest
- 严格 TS：no any, no as casts unless justified

### Python (uv/pip, pytest)
- 有 [tool.uv] 用 uv，否则 pip
- 测试用 pytest -xvs
- Type hints 仅用于 public APIs
